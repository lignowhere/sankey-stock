...
(function sankeymatic(glob) {
  ...
  glob.togglePanel = (panel) => {
    ...
    const hintEl = el(`${panel}_hint`);
    if (hintEl) hintEl.textContent = newVals.suffix;
    const indicatorEl = el(`${panel}_indicator`);
    if (indicatorEl) indicatorEl.textContent = newVals.action;
    return null;
  };
...
${ep(xcp2)} ${ep(tyC)} ${ep(tStart)} ${ep(tyC)}`
  ...
  glob.nudgeColorTheme = (themeKey, move) => {
    ...
    if (!themeOffsetEl) return null; // Added safety check
    const currentOffset = themeOffsetEl.value,
    ...
    const themeRadioEl = el(`theme_${themeKey}_radio`);
    if (themeRadioEl) themeRadioEl.checked = true; // Added safety check
  ...
  function render_sankey(allNodes, allFlows, cfg, numberStyle) {
    ...
    if (newMax !== oldMax) {
      ...
      if (cfg.labelposition_breakpoint > newMax
        ...
        const breakpointEl = el(breakpointField);
        if (breakpointEl) { // Added safety check
          breakpointEl.value = newMax;
        }
...
M${ep(n.lastPos.x)} 0 v${ep(graph.h)} m${ep(n.dx)} 0 V0`)
  ...
  function generateLink() {
    ...
    currentUrl.search
      = `${urlInputsParam}=${encodeURIComponent(compressed).replaceAll('-', '%2D')
      }`;
    return currentUrl;
  }
  ...
  glob.openGetLinkDialog = () => {
    ...
    if (dEl) {
      ...
      if (tEl) tEl.innerText = diagramUrl.toString(); // Added safety check
      if (tEl) tEl.focus(); // Added safety check
    }
  };

  glob.copyGeneratedLink = () => {
    if (glob.navigator?.clipboard) {
      const linkEl = el(linkTargetDiv);
      if (linkEl) { // Added safety check
        glob.navigator.clipboard.writeText(linkEl.innerText);
        const copiedMsgEl = el(copiedMsgId);
        if (copiedMsgEl) { // Added safety check
          copiedMsgEl.innerText = 'Copied!';
          setTimeout(() => { copiedMsgEl.innerText = ''; }, 2000);
        }
      }
    }
  };
...
style="background-color: ${swRGB};">&nbsp;</span>`
      ...
      for (const t of colorThemes.keys()) {
        const theme = approvedColorTheme(t),
          themeOffsetEl = el(offsetField(t));

        if (!themeOffsetEl) continue; // Skip if this theme's offset input is missing

        const colorset = rotateColors(theme.colorset, themeOffsetEl.value),
        ...
        // in use?
        const guideEl = el(`theme_${t}_guide`);
        if (guideEl) guideEl.innerHTML = renderedGuide;
        const labelEl = el(`theme_${t}_label`);
        if (labelEl) labelEl.textContent = theme.nickname;
      }
    }
